...
def project(
    ...
    mask = (
        (points_uv_raw[:, -1] > 0)  # must be in front of camera
        & (points_uv_raw[:, -1] < 25)  # don't render too far
...
class TrackProcessor:
    ...
    def __call__(self, sample: dict):
        ...
        # little bit of a hack, move camera slightly up for better projection
...
class EgoTrackProcessor:
    ...
    def from_frame(
        ...
        **kwargs,
        ...
        # project to bev
        waypoints = waypoints[:, [0, 2, 3]]
        track_left = track_left[:, [0, 2, 3]]
        track_right = track_right[:, [0, 2, 3]]

        # remove homogeneous coordinate
        track_left = track_left[:, :2]
        ...
        return {
            "bev_track_left": track_left.astype(np.float32),
            "bev_track_right": track_right.astype(np.float32),
    ...
    def __call__(self, sample: dict):
        ...
        waypoints = np.concatenate([waypoints, np.ones_like(waypoints[:, :1])], axis=1)
